...
OutlineCornerLocal::OutlineCornerLocal(int roadId, double s, double t, double u, double v, double zLocal, double height, double heading, int cornerId, double x = NAN, double y = NAN, double z = NAN)
    : roadId_(roadId),
      s_(s),
      t_(t),
      u_(u),
...
void Marking::FillMarkingPoints(const Point2D& point1, const Point2D& point2, OutlineCorner::CornerType cornerType)
{
    ...
    if (nrOfPoints != 0)
    {
        ...
        if (nrOfPoints >= 4)
        {
            ...
            points.push_back( GetPoint(point, cornerType));  // point_ A
            pointTemp.x = point.x + deltaP0Far;
            pointTemp.y = point.y + deltaP1Far;
            points.push_back( GetPoint(pointTemp, cornerType));// point_ B
            ...
            pointTemp.x = point.x + deltaP0Far;
            pointTemp.y = point.y + deltaP1Far;
            points.push_back( GetPoint(pointTemp, cornerType)); // point_ C
            points.push_back( GetPoint(point, cornerType));      // point_ D
        }
        ...
        for (int i = 4; i < nrOfPoints; i += 4)  // loop from second block
        {
            ...
            pointTemp.x = point.x + deltaP0Far;
            pointTemp.y = point.y + deltaP1Far;
            points.push_back( GetPoint(pointTemp, cornerType));  // point_ D
            points.push_back( GetPoint(point, cornerType));      // point_ C
        }
        markingsPoints_.push_back(points);
    }
}
    ...
    if (cornerReferenceIds.size() > 0)  // no corner ids
    {
        ...
        if (cornerReferences.size() >= 2)  // corner referrence found
        {
            for (size_t i = 0; i < cornerReferences.size() - 1; i++)
            {
                ...
                double u2, v2;
                RotateVec2D(static_cast<roadmanager::OutlineCornerLocal*>(cornerReferences[i])->GetU() * repeatScales.scale_x,
                            static_cast<roadmanager::OutlineCornerLocal*>(cornerReferences[i])->GetV() * repeatScales.scale_y,
                            total_heading,
                            u2,
                            v2);
                ...
                double u3, v3;
                RotateVec2D(static_cast<roadmanager::OutlineCornerLocal*>(cornerReferences[i + 1])->GetU() * repeatScales.scale_x,
                            static_cast<roadmanager::OutlineCornerLocal*>(cornerReferences[i + 1])->GetV() * repeatScales.scale_y,
                            total_heading,
                            u3,
                            v3);
...
void Marking::FillPointsFromObject(double s, double t, double length, double width, double objHOffset)
{
    Point2D               point1;
    Point2D               point2;
    roadmanager::Position pos;
...
void Marking::FillPointsFromOutlines(std::vector<std::shared_ptr<Outline>> outlines)
{
    for (const auto& outline : outlines)
    {
        ...
        {
            std::vector<OutlineCorner*> cornerReferences;
            GetCorners(cornerReferenceIds, outline.get(), cornerReferences);
            if (cornerReferences.size() >= 2)  // corner referrence found
            {
...
void Marking::FillPointsFromLocalOutlineRepeat(std::vector<std::shared_ptr<Outline>> outlines, std::vector<roadmanager::Repeat*> repeats)
{
    for (auto& repeat : repeats)
    {
        for (const auto& outline : outlines)
        {
            for (const auto& repeatScale : repeat->repeatScales_)
            {
                FillPointsFromScales(outline.get(), repeatScale);  // fill local corner
            }
        }
    }
}
...
void Marking::CreateMarkingsPoints(RMObject* object)
{
    if(object->GetNumberOfRepeats() > 0)
    {
        ...
        {
            FillPointsFromOutlines(object->GetOutlinesCopies());
        }
        ...
        {
            FillPointsFromRepeatsScales(object->GetRepeats(), object->GetLength(), object->GetWidth());
        }
    }
    ...
    {
        FillPointsFromObject(object->GetS(), object->GetT(), object->GetLength(), object->GetWidth(), object->GetHOffset());
    }
}
...
void RMObject::CreateOutlineCopies(  Repeat* repeat, double cur_s,  double factor,  double lengthOutline, double widthOutline, double zOutline, double heightOutline, std::vector<std::vector<Outline::point>> localPoints, int r_id )
{
    ...
    double scale_z        = 1.0;
    double scale_h       = 1.0;
    repeat->GetBBDetails(factor,  cur_length, cur_width, cur_z, cur_height);
    for (size_t i = 0; i < GetNumberOfOutlines(); i++)
    {
        const auto outlineOriginal = GetOutline(i);
        std::shared_ptr<Outline> outline = std::make_shared<Outline>(k,
                                                Outline::FillType::FILL_TYPE_UNDEFINED,
                                                outlineOriginal->GetAreaType());  // new outline for each segment
        for (size_t j = 0; j < outlineOriginal->corner_.size(); j++)
        {
            ...
            if (repeat->GetLengthStart() > SMALL_NUMBER || repeat->GetLengthEnd() > SMALL_NUMBER)
            {
            ...
            if (repeat->GetWidthEnd() > SMALL_NUMBER || repeat->GetWidthStart() > SMALL_NUMBER)
            {
            ...
            if (repeat->GetZOffsetEnd() > SMALL_NUMBER || repeat->GetZOffsetStart() > SMALL_NUMBER)
            {
...
int RMObject::checkAndCreateOutlineRepeatDetails(int r_id)
{
    ...
    for (auto& repeat : GetRepeats())
    {
        ...
        CreateBoundingBoxFromCorners(localPointsList, lengthOutline, widthOutline, zOutline, heightOutline);
        ...
        if (repeatLength > SMALL_NUMBER)
        {
            ...
            while (!(IsEqualDouble(cur_s + SMALL_NUMBER, repeatLength) || cur_s > repeatLength))
            {
                ...
                if (createShallowCopy)
                {
                    ...
                    scale.heading = pos.GetH();
                    scale.hOffset =
                        (GetOrientation() == roadmanager::Signal::Orientation::NEGATIVE ? M_PI : 0.0) + GetHOffset();
                    repeat->repeatScales_.emplace_back(std::move(scale));  // store points for shallow copy
                }
                else
                {
                    CreateOutlineCopies(repeat, cur_s, factor, lengthOutline, widthOutline, zOutline, heightOutline, localPointsList, r_id);
                }
...
void Repeat::GetBBDetails(double factor,  double& length, double& width, double& z, double& height)
{
    double repeatLength =
    ...
    if (GetZOffsetEnd() > SMALL_NUMBER || GetZOffsetStart() > SMALL_NUMBER)
    {
        z = (GetZOffsetStart() + (factor * (GetZOffsetEnd() - GetZOffsetStart())));
    }
    if (GetHeightStart() > SMALL_NUMBER || GetHeightEnd() > SMALL_NUMBER)
    {
        height = (GetHeightStart() + (factor * (GetHeightEnd() - GetHeightStart())));
    }
}
            ...
            for (pugi::xml_node object = objects.child("object"); object; object = object.next_sibling("object"))
            {
                ...
                for (pugi::xml_node repeat_node = object.child("repeat"); repeat_node; repeat_node = repeat_node.next_sibling("repeat"))
                {
                        ...
                        outlines_node_ == NULL)  // create outline when no outline in the object with repeat. Outline with repeat handed separately/
                    {
                        ...
                        std::shared_ptr<Outline> outline =
                            std::make_shared<Outline>(ids, Outline::FillType::FILL_TYPE_UNDEFINED, Outline::AreaType::CLOSED);
                ...
                if (outlines_node != NULL)
                {
                    for (pugi::xml_node outline_node = outlines_node.child("outline"); outline_node; outline_node = outline_node.next_sibling())
                    {
                        ...
                        std::shared_ptr<Outline> outline = std::make_shared<Outline>(id, Outline::FillType::FILL_TYPE_UNDEFINED, areaType);
